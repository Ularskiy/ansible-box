...
Этот проект — моя личная шпаргалка. Он не запускается, а создан для того, чтобы всегда можно было вспомнить, как организовать Ansible правильно и понятно.
...
Ты разбиваешь всё на отдельные сценарии:
...
В эту директорию можно положить **собственные модули Ansible на Python**, если тебе не хватает встроенных.

Ansible при выполнении playbook сначала ищет модули здесь, а уже потом в стандартной библиотеке.

---

### Когда использовать `library/`?

- Когда тебе нужен модуль, которого нет в Ansible
- Когда нужно взаимодействовать с кастомным API
- Когда ты хочешь написать свою реализацию, например:
  - модуль для управления железом
  - модуль для кастомной авторизации
...
Поддерживает тот же интерфейс, что и обычные модули Ansible.  
Можно использовать внутри ролей или playbook'ов:

```yaml
- name: Выполнить собственный модуль
  custom_api_call:
    endpoint: "https://..."
    token: "{{ api_token }}"
```
